    ...
    def lookup(self, license=None, email=None):
        ...
        if license is not None:
            profile = requests.get(
                f"https://laravel-api.usacycling.org/api/v1/admin/search/profiles?rider_lookup=true&exact_comp_id={license}",
                cookies=self.token.cookies,
            )
        elif email is not None:
            profile = requests.get(
                f"https://laravel-api.usacycling.org/api/v1/admin/search/profiles?rider_lookup=true&exact_email={email}",
                cookies=self.token.cookies,
            )
        else:
...
def get_random_upload_path(upload_dir, filename):
    ...
    filename = f"{uuid.uuid4()}-{randid}.{ext}"
